...
from evalution.cal_mAP import json_map
from evalution.cal_PR import json_metric, metric, json_metric_top3
...
dtd_classes = ('banded','blotchy','braided','bubbly','bumpy','chequered',
...
def evaluation(result, types, ann_path):
    ...
    for i, _ in enumerate(tqdm(classes)):
        ap = json_map(i, pred, ann_json, types)
        aps[i] = ap
    # OP, OR, OF1, CP, CR, CF1 = json_metric(pred, ann, len(classes), types)
