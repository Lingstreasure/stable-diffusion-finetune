...
class DInterface(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        if stage == 'fit' or stage is None:
            self.kwargs['file_list_path'] = self.kwargs['train_list_path']
            self.trainset = self.instancialize(train=True, **self.kwargs)
            self.kwargs['file_list_path'] = self.kwargs['test_list_path']
            self.valset = self.instancialize(train=False, **self.kwargs)
            print("Train set num : {}".format(len(self.trainset)))
            print("Validation set num : {}".format(len(self.valset)))

        # Assign test dataset for use in dataloader(s)
        if stage == 'test' or stage is None:
            self.testset = self.instancialize(train=False, **self.kwargs)
